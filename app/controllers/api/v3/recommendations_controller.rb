class Api::V3::RecommendationsController < ApplicationController
  ...
  def create
    ...
    if params["restaurant_id"].length <= 9 && Recommendation.where(restaurant_id:params["restaurant_id"].to_i, user_id: @user.id).length > 0
      ...
      restaurant_info = JSON(Nokogiri.HTML(open("http://www.needl.fr/api/v3/restaurants/#{@recommendation.restaurant_id}.json?user_email=#{@user.email}&user_token=#{@user.authentication_token}")))
