class WishlistsController < ApplicationController
  ...
  def create
    ...
    if Wishlist.where(restaurant_id:params["restaurant_id"].to_i, user_id: current_user.id).any? || Wishlist.where(restaurant_id:params["wishlist"]["restaurant_id"].to_i, user_id: current_user.id).any?
      redirect_to restaurants_path, notice: "Restaurant déjà sur ta wishlist"
  ...
  def identify_or_create_restaurant
    ...
    # si la wishlist se fait directement depuis la page du resto
    if params["id"]
      @restaurant = Restaurant.find(params["id"])

    # si elle se fait depuis la page d'ajout d'adresse classique
    elsif  params[:restaurant_origin] == "db" || params[:restaurant_origin] == "foursquare"
